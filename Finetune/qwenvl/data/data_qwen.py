...
class LazySupervisedDataset(Dataset):
    ...
    def process_image_egra(self, image_file,image_gt_files):
        if image_file.split('/')[-1] in image_gt_files:
            max_pixels=self.data_args.max_pixels
            min_pixels=self.data_args.min_pixels
        else:
            max_pixels=self.data_args.low_max_pixels
            min_pixels=self.data_args.min_pixels
    
        size = {"longest_edge":max_pixels,"shortest_edge":min_pixels}
        processor = self.data_args.image_processor
        image = Image.open(image_file).convert("RGB")
        visual_processed = processor.preprocess(image, size=size,max_pixels=max_pixels,min_pixels=min_pixels,return_tensors="pt")
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources[0]:
            ...
            image_gt_file = self.list_data_dict[i]["gt_page"]
            if isinstance(image_file, List):
                if len(image_file) > 1:
                    image_gt_file = update_image_list(image_file, image_gt_file)
                    image_file = [
                        os.path.join(image_folder, file) for file in image_file
                    ]
                    if self.data_args.use_egra:
                        results = [self.process_image_egra(file,image_gt_file) for file in image_file]
                    else:
                        #print(f'self.process_image_unified')
                        #int(self.data_args.max_pixels/(28*28))
                    ...
                    if self.data_args.use_egra:
                        image, grid_thw = self.process_image_egra(image_file,image_gt_file)
                    else:
                        #print(f'self.process_image_unified')
